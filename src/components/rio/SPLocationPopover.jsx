...
function SPLocationPopover({ isOpen, anchorEl, onClose, centered = false, language = "POR" }) {
  ...
  const locationData = (language === "ENG" ? locationsDataUS : locationsDataBR).locations.sp;
  ...
  const t = {
    POR: {
      subs: "Subsidiárias diretas",
      joint: "Joint Ventures / Participações parciais",
      close: "Fechar"
    },
    ENG: {
      subs: "Direct subsidiaries",
      joint: "Joint ventures / Partial ownership",
      close: "Close"
    }
  }[language];
  ...
  const content = (
    <>
      {/* Arrow pointing left */}
      <div className="absolute left-[-7px] top-1/2 -translate-y-1/2 w-0 h-0 border-t-[7px] border-t-transparent border-b-[7px] border-b-transparent border-r-10 border-r-white z-1001" />
      <div className="md:gap-3 max-[480px]:py-2.5 max-[480px]:gap-2.5 max-[360px]:p-2 flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 max-[480px]:gap-1.5">
            ...
            <div className="text-grey-800 font-[ShellBook,'-apple-system',Roboto,Helvetica,sans-serif] text-[10px] md:text-[16px] max-[480px]:text-[14px] max-[360px]:text-xs font-normal leading-normal whitespace-nowrap">{locationData.badge}</div>
          </div>
          <h1 className="text-grey-800 font-[ShellBold,'-apple-system',Roboto,Helvetica,sans-serif] text-lg md:text-3xl max-[480px]:text-2xl max-[360px]:text-xl font-bold leading-normal m-0 whitespace-nowrap">{locationData.name}</h1>
        </div>

        {/* Direct Subsidiaries */}
        <div className="flex flex-col gap-1.5 max-[480px]:gap-2.5">
          <h2 className="text-grey-600 font-[ShellMedium,'-apple-system',Roboto,Helvetica,sans-serif] text-[10px] md:text-[16px] max-[480px]:text-[14px] max-[360px]:text-xs font-medium leading-normal m-0 whitespace-nowrap">{t.subs}</h2>
          <div className="flex flex-col gap-1 md:gap-2.5">
            {subsidiaries.map((company) => (
              <div key={company.id} className="flex items-center gap-1.5 md:gap-2 max-[480px]:flex-col max-[480px]:items-start max-[480px]:gap-1.5">
                <img
                ...
                <div className="flex flex-col gap-1 flex-1">
                  <h3 className="text-grey-800 font-[ShellBook,'-apple-system',Roboto,Helvetica,sans-serif] text-xs md:text-lg max-[480px]:text-base max-[360px]:text-sm font-normal leading-normal m-0 whitespace-nowrap">{company.name}</h3>
                  <div className="flex items-center gap-1.5 md:gap-1 whitespace-nowrap">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-grey-600 font-[ShellBook,'-apple-system',Roboto,Helvetica,sans-serif] text-[10px] md:text-[16px] max-[480px]:text-xs max-[360px]:text-[10px] font-normal leading-normal">{company.business}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-grey-600 font-[ShellMedium,'-apple-system',Roboto,Helvetica,sans-serif] text-[10px] md:text-[16px] max-[480px]:text-xs max-[360px]:text-[10px] font-medium leading-normal">{company.ownership}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Divider */}
        <div className="w-full h-px bg-grey-100 border-none m-0" />

        {/* Joint Ventures */}
        <div className="flex flex-col gap-1.5 max-[480px]:gap-2.5">
          <h2 className="text-grey-600 font-[ShellMedium,'-apple-system',Roboto,Helvetica,sans-serif] text-[10px] md:text-[16px] max-[480px]:text-[14px] max-[360px]:text-xs font-medium leading-normal m-0 whitespace-nowrap">{t.joint}</h2>
          <div className="flex flex-col gap-1 md:gap-2.5">
        ...
        {/* Close Button */}
        <button
          className="flex font-bold text-lg w-full items-center justify-center p-2 border border-grey-400 rounded-md hover:bg-grey-50 hover:border-grey-600 transition-colors outline-none focus:outline-none focus:border-grey-600 whitespace-nowrap"
          onClick={onClose}
  ...
  if (centered) {
    return createPortal(
      ...
      >
        ...
        >
          <div className="py-5 px-8 w-full border border-grey-100 rounded-2xl bg-white shadow-[0_4px_20px_rgba(0,0,0,0.15)] relative">
            {content}
          </div>
        </div>
      </div>,
...
export default SPLocationPopover;
