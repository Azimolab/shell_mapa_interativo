...
function Toolbar({
  ...
  language = 'POR' // ✅ recebe idioma
}) {
  // ✅ escolhe fonte de dados por idioma
  ...
  const t = useMemo(() => {
    if (language === 'ENG') {
      return {
        header: 'Click to change the viewing area',
        mapTitle: 'Brazil',
        maritimeBlocks: 'Offshore Blocks',
        exploration: 'In exploration',
        development: 'In development',
        production: 'On-stream',
        decommissioning: 'In decommissioning',
        visible: 'Visible',
        hidden: 'Hidden',
        infrastructure: 'Infrastructure',
        pipeline_oil: 'Oil Pipeline',
        pipeline_gas: 'Gas Pipeline',
        third_party: 'Non-Shell Pipeline',
    ...
    // PT-BR
    return {
      header: 'Clique para alterar a área visualizada',
      mapTitle: 'Brasil',
      maritimeBlocks: 'Blocos Marítimos',
      exploration: 'Em exploração',
      development: 'Em desenvolvimento',
      production: 'Em produção',
      decommissioning: 'Em descomissionamento',
      visible: 'Visível',
      hidden: 'Oculto',
      infrastructure: 'Infraestrutura',
      pipeline_oil: 'Óleoduto',
      pipeline_gas: 'Gasoduto',
      third_party: 'Infraestrutura de terceiros',
      extraInfo: 'Informações Adicionais',
      operator: 'Operador da plataforma',
      depth: 'Profundidade',
      areas: {
        barreirinhas: 'Barreirinhas',
        potiguar: 'Potiguar',
  ...
  // Mapeia estado de visibilidade vindo do pai
  ...
  // ✅ Depende do mapData escolhido
  const availableLegends = useMemo(() => {
    if (!mapData[selectedYear] || !mapData[selectedYear][selectedArea]) {
      return { exploracao: false, desenvolvimento: false, producao: false, descomissionamento: false };
    }
    const regionData = mapData[selectedYear][selectedArea];
    return {
      exploracao: !!(regionData.explorationPins && regionData.explorationPins.length),
      desenvolvimento: !!(regionData.developmentPins && regionData.developmentPins.length),
      producao: !!(regionData.productionPins && regionData.productionPins.length),
      descomissionamento: !!(regionData.decommissioningPins && regionData.decommissioningPins.length),
    };
  }, [mapData, selectedYear, selectedArea]);

  // ✅ Depende do mapData escolhido
  const availableInfrastructure = useMemo(() => {
    if (!mapData[selectedYear] || !mapData[selectedYear][selectedArea]) {
      return { oleoduto: false, gasoduto: false, terceiros: false };
    }
    const regionData = mapData[selectedYear][selectedArea];
    const infrastructure = regionData.infrastructure || {};
    return {
      oleoduto: !!infrastructure.oleoduto,
      gasoduto: !!infrastructure.gasoduto,
      terceiros: !!infrastructure.terceiros
    };
  }, [mapData, selectedYear, selectedArea]);
  ...
  const isAreaAvailableForYear = (areaId, year) => {
    ...
    switch (areaId) {
      case 'barreirinhas': return yearNum >= 2016;
      case 'potiguar': return yearNum >= 2018;
      case 'pelotas': return yearNum >= 2024;
  ...
  return (
    <div className={`idiom-${language === 'ENG' ? 'eng' : 'por'}`} data-name={`Idiom=${language}`}>
      {/* Map Selector */}
      <div className="map-selector" data-name="Map Selector">
        <div className="header" data-name="Header">
          <p className="section-title">{t.header}</p>

          <div className="menu" data-name="Menu">
        ...
        <div className="map-selector-content" data-name="MapSelector" data-node-id="13:4028">
          ...
          <p className="map-title" data-node-id="I13:4028;13:3880">{t.mapTitle}</p>
      ...
      {/* Subtitles */}
      <div className="subtitles" data-name="Subtitles">
        {/* Blocos Marítimos */}
        <div className="bloco-maritimo">
          <h3 className="section-title">{t.maritimeBlocks}</h3>

          <div className="menu">
            <div className="filter-block">
              {/* Em exploração */}
              <div className={`exploracao ${!availableLegends.exploracao ? 'disabled' : ''}`}>
                <div className="icon-text">
                  <div className="icon-container">
                    <div className="icon-box">
                      <img src={ExploracaoIcon} alt={t.exploration} />
                    </div>
                  </div>
                  <span className="legend-label">{t.exploration}</span>
                </div>
                <button
                ...
                >
                  <img src={visibleLegends.exploracao ? EyeIcon : EyeDisableIcon} alt={visibleLegends.exploracao ? t.visible : t.hidden} />
                </button>
              </div>

              {/* Em desenvolvimento */}
              <div className={`construcao ${!availableLegends.desenvolvimento ? 'disabled' : ''}`}>
                <div className="icon-text">
                  <div className="icon-container">
                    <div className="icon-box">
                      <img src={DesenvolvimentoIcon} alt={t.development} />
                    </div>
                  </div>
                  <span className="legend-label">{t.development}</span>
                </div>
                <button
                ...
                >
                  <img src={visibleLegends.desenvolvimento ? EyeIcon : EyeDisableIcon} alt={visibleLegends.desenvolvimento ? t.visible : t.hidden} />
                </button>
              </div>

              {/* Em produção */}
              <div className={`producao ${!availableLegends.producao ? 'disabled' : ''}`}>
                <div className="icon-text">
                  <div className="icon-container">
                    <div className="icon-box">
                      <img src={ProducaoIcon} alt={t.production} />
                    </div>
                  </div>
                  <span className="legend-label">{t.production}</span>
                </div>
                <button
                ...
                >
                  <img src={visibleLegends.producao ? EyeIcon : EyeDisableIcon} alt={visibleLegends.producao ? t.visible : t.hidden} />
                </button>
              </div>

              {/* Em descomissionamento */}
              <div className={`descomissionamento ${!availableLegends.descomissionamento ? 'disabled' : ''}`}>
                <div className="icon-text">
                  <div className="icon-container">
                    <div className="icon-box">
                      <img src={DescomissionamentoIcon} alt={t.decommissioning} />
                    </div>
                  </div>
                  <span className="legend-label">{t.decommissioning}</span>
                </div>
                <button
                ...
                >
                  <img src={visibleLegends.descomissionamento ? EyeIcon : EyeDisableIcon} alt={visibleLegends.descomissionamento ? t.visible : t.hidden} />
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="infraestrutura">
          <h3 className="section-title">{t.infrastructure}</h3>
          <div className="menu">
            {/* Óleoduto */}
            <div className={`oleoduto ${!availableInfrastructure.oleoduto ? 'disabled' : ''}`}>
              ...
              <span className="legend-label">{t.pipeline_oil}</span>
            </div>

            {/* Gasoduto */}
            <div className={`gasoduto ${!availableInfrastructure.gasoduto ? 'disabled' : ''}`}>
              <div className="box-icon">
                <svg className="icon" width="18" height="18" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <span className="legend-label">{t.pipeline_gas}</span>
            </div>

            {/* Infraestrutura de terceiros */}
            <div className={`terceiros ${!availableInfrastructure.terceiros ? 'disabled' : ''}`}>
              <div className="box-icon">
                <svg className="icon" width="18" height="18" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M5.5 18H31.5" stroke="#343434" strokeWidth="4" strokeLinecap="round" strokeDasharray="8 7"/>
                </svg>
              </div>
              <span className="legend-label">{t.third_party}</span>
            </div>
          </div>
        </div>
        ...
        <div className="informacoes-adicionais">
          <h3 className="section-title">{t.extraInfo}</h3>
          <div className="menu">
            {/* Operador */}
            <div className="operador">
              ...
              <span className="legend-label">{t.operator}</span>
            </div>

            {/* Profundidade */}
            <div className="profundidade">
              <div className="box-icon">
                <svg className="icon" width="18" height="18" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <span className="legend-label">{t.depth}</span>
            </div>
          </div>
        </div>
      </div>
