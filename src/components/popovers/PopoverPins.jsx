...
function PopoverPins({ isOpen, anchorEl, onClose, data, language }) {
  ...
  const [lastShellPercent, setLastShellPercent] = useState(0);
  const [animateShell, setAnimateShell] = useState(false);
  const popoverRef = useRef(null);

  // 2) REINICIAR ANIMAÇÃO QUANDO TROCAR DE PIN (ou reabrir)
  useEffect(() => {
    // sempre que abrir e mudar de dado, zera e arma o gatilho
    if (isOpen && data) {
      setAnimateShell(false);
      // dois RAFs garantem que a largura 0% seja aplicada antes de crescer
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          setAnimateShell(true);
        });
      });
    }
  }, [isOpen, data?.id, data?.title, data?.number]);

  // ✅ Dicionário multilíngue
  const translate = (key) => {
    const dict = {
      Operadora: { POR: "Operadora", ENG: "Operator" },
      Profundidade: { POR: "Profundidade", ENG: "Depth" },
      ParticipacaoEmpresas: {
        POR: "Participação das empresas",
        ENG: "Companies Participation",
      },
      InfraestruturaAssociada: {
        POR: "Infraestrutura associada",
        ENG: "Associated Infrastructure",
      },
      Fechar: { POR: "Fechar", ENG: "Close" },
      StatusNaoDefinido: {
        POR: "Status não definido",
        ENG: "Undefined status",
      },
    };
    return dict[key]?.[language] ?? dict[key]?.POR ?? key;
  };
  ...
  const calculateArrowPosition = (anchorRect, bestPosition, popoverElement) => {
    ...
    const actualSide = Object.entries(distances).reduce(
      (max, [side, dist]) => (dist > max.dist ? { side, dist } : max),
      { side: "bottom", dist: -Infinity }
    ).side;
    ...
    if (actualSide === "top" || actualSide === "bottom") {
      ...
      if (relativeX < minX || relativeX > maxX) {
        ...
        console.log(
          "⚠️ Arrow foi limitada (popover pode não estar bem posicionado)"
        );
      }
    ...
    if (anchorEl && isOpen) {
      ...
      requestAnimationFrame(() => {
        ...
        if (rect.width === 0 && rect.height === 0) {
          console.warn(
            "⚠️ AnchorEl com dimensões inválidas, tentando novamente..."
          );
          ...
          setTimeout(() => {
            ...
            const bestPosition = calculateBestPopoverPosition(newRect, {
              width: 500,
              height: 600,
            });
          ...
          // Calcular melhor posição
          const bestPosition = calculateBestPopoverPosition(rect, {
            width: 500,
            height: 600,
          });
          setPopoverPosition(bestPosition);
  ...
  const status =
    data.status?.label || data.status || translate("StatusNaoDefinido");
  const operatorName = data.operator?.name || data.operator || null;
  const operatorDescription =
    data.operator?.description || data.operatorDescription || null;
  const depthValue = data.depth?.value || data.depth || null;
  const depthDescription =
    data.depth?.description || data.depthDescription || null;
  ...
  const shouldShowSecondDivider =
    (hasCompanies || hasSummary) && hasInfrastructure;
  ...
  const content = (
    <>
      ...
      <div className="popover-pins-card">
                ...
                <div className="popover-pins-info-block">
                  <div className="popover-pins-info-header">
                    <div className="popover-pins-info-icon-container">
                      <svg
                        className="popover-pins-info-icon"
                        width="36"
                        height="36"
                        viewBox="0 0 36 36"
                        fill="none"
        ...
        {shouldShowFirstDivider && <div className="popover-pins-divider"></div>}

        {hasCompanies && (
          
          <div className="popover-pins-section">
            <h2 className="popover-pins-section-title">
              {translate("ParticipacaoEmpresas")}
            </h2>
            <div className="popover-pins-companies">
              {data.companies.map((company, index) => {
                ...
                return (
                  <div key={`${data?.id || data?.title || ''}-${index}`}  className="popover-pins-company">
                    <div className="popover-pins-company-header">
                      <span
                        className={`popover-pins-company-name ${
  ...
  <div
    className={`popover-pins-progress-fill ${isShell ? 'is-shell' : 'no-anim'}`}
    // Shell: parte de 0 e cresce até o valor; outras já nascem completas e sem transição
    style={{
      width: isShell
        ? (animateShell ? `${company.percentage}%` : '0%')
        : `${company.percentage}%`
    }}
  />
</div>
        ...
        {hasInfrastructure && (
          <div className="popover-pins-section">
            <h2 className="popover-pins-section-title">
              {translate("InfraestruturaAssociada")}
            </h2>
                ...
                <div className="popover-pins-bullets">
                  {infrastructureItems.map((item, index) => (
                    <div key={item.id || index} className="popover-pins-bullet">
                      ...
                      <span className="popover-pins-bullet-text">
                        {item.name}
                      </span>
                    </div>
                  ))}
